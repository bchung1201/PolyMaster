...
from agents.data.polymarket.gamma import GammaMarketClient as Gamma
from agents.data.news.chroma import PolymarketRAG as Chroma
from agents.models.schemas import SimpleEvent, SimpleMarket
...
class Executor:
    def __init__(self, default_model='gpt-4o-mini') -> None:
        load_dotenv()
        max_token_model = {
    ...
    def filter_events_with_rag(self, events: "list[tuple[SimpleEvent, float]]") -> "list[tuple[SimpleEvent, float]]":
        ...
        for event_tuple in events:
            event_data = event_tuple[0]
            # Handle both old format (direct event) and new format (dict with event and trade)
